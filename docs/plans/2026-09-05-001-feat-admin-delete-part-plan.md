...
### Resolution notes (post-implementation)

- **R10 vs KTD-2 destructive styling contradiction — RESOLVED at U3 implementation.** The implementation picks **outline-destructive for Delete + Cancel** (low-commitment states; honors KTD-2's hierarchy argument) and **filled-destructive for Confirm Delete** (commitment-gated action; honors R10's "filled" intent). The plan's original entries are now reconcilable as a context-conditional reading: outline when the action is reversible (Cancel) or pre-confirmation (Delete), filled when the action is committed (Confirm). Documented here so a future reader of the original R10/KTD-2 entries doesn't see the apparent conflict and wonder which to follow.

### From 2026-09-05 code review

- **PHP-side round-trip test for SKUs containing `/` — M8 (P2, ce-correctness-reviewer).** The hub's `deletePart` URL-encodes `/` to `%2F` (covered at `tests/admin/parts.test.ts:527-536`) and the hub-side `getPart`/`updatePart` already encode likewise, but no PHP-side test asserts that Laravel's `Route::bind` callback (`bootstrap/app.php:19`) receives the decoded SKU and matches the seeded row. This is a pre-existing gap on `update` and `show` too — only `destroy` was added in this plan. Fix: add a fixture helper that seeds `ABC/123` and a parametrized test that issues GET/PATCH/DELETE for `ABC%2F123`, asserting all three return 2xx and the row is mutated/deleted as expected. Out of scope for this plan; carry as the first item of a future test-coverage plan.

- **`destroy()` admin-gate ordering** — RESOLVED at U1 commit `a6e37e6`+`f8…` (post-review polish). The implementation already runs `adminTokenIsValid` before `$part->delete()`, but a code-review comment now documents the invariant at `PartController::destroy()` so a future refactor can't silently reorder. The `test_destroy_without_admin_token_returns_403` case asserts this via `assertDatabaseHas` on the 403 path; no further fix needed.

- **Hub-side `revalidatePath` failure not handled** — RESOLVED at U2 commit `9956dfe`+post-review polish. The action now wraps `revalidatePath('/admin/inventory')` in `try/catch` with a `console.error` log; on cache-invalidation failure the action still returns `{ok: true, redirectTo}` so the UI navigates away from a row the PHP API has already deleted. Previously a throw here would leave the user on the edit page showing stale state. Documented in code; no behavior change for the happy path.

- **`DeleteState` type didn't model the action's `fieldErrors` branch** — RESOLVED at post-review polish. The action's failure branch was `{ok: false; fieldErrors: {}, error?: string}`, but the `DeleteState` type in `PartForm.tsx` declared only `{ok: false; error?: string}` (no `fieldErrors`). TypeScript's structural typing allowed the assignment (excess properties are silently dropped on cross-function returns), but a future contributor reading `state.fieldErrors` from the action's actual return shape would find it missing from the consumer-side type. Fix: dropped `fieldErrors: {}` from `deletePartAction`'s return type since the action never populates it (no Zod gate, no Laravel validation surface on DELETE). `DeleteState` is now the single source of truth.

- **R11 Cancel-button not disabled during pending** — RESOLVED at post-review polish. R11 reads "disable the active Delete-form button (Cancel or Confirm Delete — only one is visible at a time) with `aria-busy='true'` while the action is pending." The implementation initially disabled only Confirm; Cancel could be clicked mid-pending, flipping the UI back to the Delete button while the in-flight server action still resolved and the success-navigate effect overrode the user's intent. Fix: `DeleteForm` now tracks `pending` locally via `ConfirmDeleteSubmit.onLocalPendingChange`, and both Cancel and the restored Delete button disable while pending. Documented inline at the Cancel button.
